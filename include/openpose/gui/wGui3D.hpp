            ...
            if (tDatums != nullptr)
            {
                ...
                if (!tDatums->empty())
                {
                    ...
                    for (auto& tDatumPtr : *tDatums)
                       {
                        //putTextOnCvMat(tDatumPtr->cvOutputData, "handKeypoints:",
                        //{int(tDatumPtr->handKeypoints3D[0][0]), int(tDatumPtr->handKeypoints3D[0][1])}, WHITE_SCALAR, false, tDatumPtr->cvOutputData.cols);
